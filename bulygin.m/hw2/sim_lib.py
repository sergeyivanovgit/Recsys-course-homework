...
def simulation(policy: Callable, n=10 ** 6, initial_banners=9):
    ...
    # store simulation results on each iteration
    simulation_results = {
        'regret': [regret],
        'n': [0],
    ...
    for i in range(n):
        ...
        # tracking simulation results on current iteration
        simulation_results['regret'].append(regret)
        simulation_results['n'].append(i + 1)
        simulation_results['total_banners'].append(max_index)
    ...
    return {'regret': regret, 'rounds': n, 'total_banners': max_index, 
            'history': state, 'results': simulation_results}